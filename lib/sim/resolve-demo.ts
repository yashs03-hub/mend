 ...
 * links freeze `/family?state=attention` and `/call?stage=escalated` — then
 * the console's active-scenario store, then green.
 *
 * Family's `state` vocabulary (well / attention) is not the same as the
 * store's scenario vocabulary (green / pe / drift). `attention` maps to the
 * drift fixture so the amber chip still comes from `composeDecision` over
 * the trend engine, never from a hard-coded severity.
 */
